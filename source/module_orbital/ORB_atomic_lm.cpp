...
void Numerical_Orbital_Lm::use_uniform(const double &dr_uniform_in)
{
    ...
    {
        this->psi_uniform[i] = ModuleBase::Mathzone_Add1::Uni_RadialF(ModuleBase::GlobalFunc::VECTOR_TO_PTR(psi),
                                                                      this->nr,
                                                                      this->rab[0],
                                                                      i * dr_uniform);
    }
    ...
    ModuleBase::Mathzone_Add1::Uni_Deriv_Phi(ModuleBase::GlobalFunc::VECTOR_TO_PTR(psi_uniform),
                                             nr_uniform,
                                             dr_uniform,
                                             1,
                                             ModuleBase::GlobalFunc::VECTOR_TO_PTR(dpsi_uniform));
    ...
    {
        std::stringstream ss;
        ss << GlobalV::global_out_dir << this->label << "/" << this->label << "-" << orbital_type
           << ".ORBITAL_NOR_uniform.txt";
...
void Numerical_Orbital_Lm::plot(void) const
{
    ...
    if (GlobalV::MY_RANK == 0)
    {
        ...
        ssr << GlobalV::global_out_dir << this->label << "/" << this->label << "-" << orbital_type << index_chi + 1
            << "-orbital-r.dat";

        ssk << GlobalV::global_out_dir << this->label << "/" << this->label << "-" << orbital_type << index_chi + 1
            << "-orbital-k.dat";

        ssru << GlobalV::global_out_dir << this->label << "/" << this->label << "-" << orbital_type << index_chi + 1
             << "-orbital-ru.dat";
